...
func test_movement_camera_scoring_and_weather_resources_load() -> void:
	assert_eq(tuning.movement().move_speeds[&"sprint"], 6.0)
	assert_eq(tuning.movement().material_noise_multipliers[&"tatami"], 0.5)
	assert_eq(tuning.camera().mouse_look_sensitivity, 0.0025)
	assert_eq(tuning.camera().gamepad_look_speed, 2.5)
