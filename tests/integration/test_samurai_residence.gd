...
func test_residence_route_geometry_keeps_veranda_and_crawlspace_open() -> void:
	...
	var crawl_points := residence.route_waypoints(&"C_crawlspace")
	assert_true(
		residence._route_points_have_support(crawl_points),
		"C crawl waypoints must have world collision support",
	)
	assert_true(
		residence._route_segments_are_clear(crawl_points, &"C_crawlspace"),
		"C crawl segments must clear the authored crawl roof",
	)
	assert_true(
		residence._route_navigation_covers(crawl_points, &"C_crawlspace"),
		"C crawl waypoints must be covered by CrawlspaceNavigation",
	)
