...
`assets/audio/rain.wav` is original deterministic synthesized noise generated by `tools/audio/build_rain.py`, under the project license, without recordings or external samples. Rain/snow particles and footprint meshes are procedural geometry authored in `src/stealth/weather_presentation.gd`; no external art dependency is added.
...
## Original port modules (#61)

`assets/environment/port_modules.glb` and its JSON manifest are generated by
`tools/art/build_port_modules.py` in Blender 5.2 LTS. Quays, plaster walls, tiles,
shoji, ship hull/rigging, cargo, lamps, counting cabinet and clothed dock workers
use original geometry and materials. No external models, images, recordings or
franchise assets are included. The manifest's project-license label records
provenance; this asset pass does not introduce a new repository license grant.
