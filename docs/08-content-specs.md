...
| visibility_mods | Dictionary[Stance→float] | sneak 0.6 / walk 1.0 / sprint 1.3 / crawl 0.3 / swim 0.2 |
...
| display_name_key | StringName | tool.stone | tool.dart | tool.smoke | tool.naruko | tool.rope |
...
| objectives | Array[ObjectiveData] | {id, text_key, kind: KILL_TARGET/STEAL/RESCUE/ESCAPE/PROTECT, target_group} 順序付き。表示文言は `data/text/ja.csv` から解決 |
| side_objective | ObjectiveData | null 可（M8 / M10）。それ以外は原則必須 |
...
if stim == null:
  # 刺激がないフレームは routine/timer 継続のみ。dummy stimulus や P0 sentinel は作らない。
  match state:
    UNAWARE: run_routine()
    SUSPICIOUS: continue_investigation(delta)
    SEARCHING: continue_search(delta)
    RETURN: continue_return(delta)
    COMBAT: continue_combat(delta)
  return
...
### Enemy ninja runtime extension (#57)

`EnemyNinja` inherits EnemyBase and preserves its child APIs. Its perception_shinobi row and enemy_shinobi stats select130-degree FOV, hearing1.5,4HP/1damage, dart immunity, plus Brain knockout immunity and zero relight delay. Ground navigation stays layer1; authored ninja roof regions/links use layer2 and ninja agents1|2. `CaltropTrap` uses player-only overlap damage once,30s life,10s placement cooldown and3-per-ninja/12-world caps. Shared navigation retains progress for unchanged destinations and initializes empty paths, with bounded1mm collision-margin recovery. Ninja waypoint proximity is1mm for vertical exits. Actual displacement drives enemy locomotion/climb animation; V remains the common visibility formula.
